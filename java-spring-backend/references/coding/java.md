...
日志级别应与实际影响匹配，不要把正常业务分支大量记录为 `error`。
...
对公共接口、抽象能力、容易误用的方法，如果仅靠签名无法表达完整契约，应补充有效 Javadoc。

不机械要求每个类都添加作者、创建日期，也不为了满足注释数量编写无信息量注释。

不要保留已经废弃的大段注释代码；历史实现应交给版本控制系统保存。
...
20. 只报告和执行项目真实存在的 formatter、静态检查和测试。
