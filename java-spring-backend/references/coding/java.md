...
普通业务查询条件超过 3 个时，优先评估封装为 Query 对象，避免使用大量方法参数或：
...
21. 遵守 4 空格、UTF-8、LF、注释间距和逻辑空行等默认格式；项目 formatter 存在时以项目配置为准。
22. 修改代码行为时同步检查注释和 Javadoc，禁止保留与实现不一致的旧注释。
23. 不长期保留无说明的注释代码，不机械添加作者、日期或无信息量注释。
24. 新增日志时检查日志门面、参数化输出、级别、异常现场、重复日志、日志量和敏感信息。
25. 不无需求修改项目日志级别、Appender、日志文件名、采集方式或保留周期。
26. 只报告和执行项目真实存在的 formatter、静态检查和测试。
